    ...
    <Section id="experience" title={content.title} description={content.description}>
      <ol className="mt-10 space-y-8 sm:mt-12 lg:mt-16">
        {content.items.map((item) => (
          <li
            className="relative grid gap-3 border-l border-line pb-8 pl-6 last:pb-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:gap-x-8 sm:pl-8"
            key={item.company}
          >
            ...
            <a
              className="section-link sm:pt-1"
              href={item.website}
              target="_blank"
              rel="noreferrer"
            >
